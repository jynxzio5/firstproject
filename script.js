...
class Particle {
    ...
    update(mouseX, mouseY) {
        ...
        // Adjust speed based on distance from mouse
        if (distance < 100) {
            this.speedX += dx * 0.01; // Move away from mouse
            this.speedY += dy * 0.01; // Move away from mouse
        }
...
const animate = (mouseX, mouseY) => {
    ...
    for (let i = 0; i < particles.length; i++) {
        particles[i].update(mouseX, mouseY); // Update particle position
        particles[i].draw();   // Redraw particle
    }
    connectParticles();       // Draw lines between particles
    requestAnimationFrame(() => animate(mouseX, mouseY)); // Call animate again to create a loop
};
...
window.addEventListener('mousemove', (event) => {
    mouseX = event.x; // Update mouseX position
    mouseY = event.y; // Update mouseY position
});
...
animate(mouseX, mouseY); // Start the animation loop
